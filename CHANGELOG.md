...
- 撤回Build84额外添加的导航、底部标签、我的列表名称及菜单选中项的实色底板/竖条，按官方v1.9.0恢复文字、图标与箭头表达选中态。表头维持透明；上游原有的输入框、设置分页和歌曲多选底色保留，不进行全局背景清空。
- 正在播放颜色同时检查对页面的可读性、与普通歌名/辅助文字的Oklab色差和色度范围。修正仅对背景提高对比度，导致浅色主题接近黑色、深色主题变为白色的问题。彩色主题优先保留自身色相；中性主题使用适合底色的克制彩色强调，不再回退为黑/白/灰。
- 不给播放状态增加整行填充、粗体或竖条；歌名、歌手、专辑、时长与播放标记使用同一强调色。保留来源标记、播放列表自动和手动定位；键盘焦点/多选与播放状态分别处理。
- 新增视觉策略回归，覆盖实际iOS主题可读性变换后的16个内置主题、菜单背景归属、真实SongRowContent渲染、透明底色和同色自定义文字。调整旧测试中要求中性主题必须使用灰色的错误断言。
- 模拟器表格截图将正在播放行放进首屏，并记录主题、普通/播放文字色及色差；扩展为54个生产组件视图，单独上传截图/运行记录，不能再用未取回的截图宣称人工验收。
- 模拟器验证发现UIKit窗口样式对象的地址和isEqual均不能判断样式；两个对象都报告minimal却比较不相等。固定SDK的测试桥接改为验证描述中的实际类型，并同时核对minimal与automatic工厂的类型，格式未知即失败；设备生产代码不解析描述、不访问私有属性。
- 模拟器编译显式使用与设备Archive相同的iPhone/iPad双设备类型，增加Info.plist和运行时deviceIdiom检查，防止在iPad模拟器中运行手机版兼容窗口而冒充横屏验收。
- 音频、音量、缓存、同步认证和原生Scene实现不变；按Build85源码指纹检查，沿用完整原生播放/同步和类型检查。编译及实际截图结果以本轮Actions为准，不预先声明真机或任意背景图片上的视觉验收。


## iOS / iPadOS 1.9.0 Build 85 · 2026-09-14

- 数据同步：逐项对照官方 Android v1.9.0 的 `/hello → /id → /ah → /socket` 认证。保持 v4、MD5前16位十六进制字符的UTF-8密钥、AES/ECB/PKCS7、RSA-OAEP/SHA1、SPKI/PKCS8、公钥/设备名/客户端类型的换行格式及 `cg_` 消息格式，不修改电脑端协议。
- iOS 认证改用已有异步 AES/RSA 原生接口，逐方法检查导出；认证编码、配对摘要与压缩采用显式纯 JavaScript Buffer、SparkMD5、pako，移除同步路径对阻塞导出、JSI摘要、原生后台定时器和原生文件压缩方法的强制依赖。请求支持超时与取消，旧连接的迟到加密结果不得新建套接字。
- 对齐 React Native 0.73 项目的 Base64 依赖：统一为旧架构兼容的2.2.2，消除 Buffer 内嵌3.0.1与顶层2.2.2的重复版本。音频引擎、音量、音频缓存和 Scene 原生实现不变。
- 同步错误按阶段显示，并提供不含密码、配对码和密钥的复制诊断；消除同一错误重复显示。仅截图的 `undefined is not a function` 不能唯一指认设备上的缺失方法，不把它默认归因于防火墙。
- 正在播放的行改为主题自适应前景与播放标记：保留主题色相，限制饱和度，针对实际配置底色/弹窗底色计算对比度。不锁定蓝色，不使用播放状态整行填充、粗体或左侧竖条；多选与键盘焦点单独区分，自动定位和手动定位保留。
- 键盘配置整合为设置页入口与窗口居中、可滚动的配置弹窗。既有总开关、播放/进度/选择/导航分项和保存值保留。
- 弹窗取消通用的强制位置转换。只有播放器播放列表显式使用左侧；历史地址等按调用方原始位置，键盘配置与收藏/确认居中；横屏播放设置和音效在底部，菜单维持触发按钮旁且无整窗暗幕。
- 设置核对：Build83的91个设置TS/TSX文件均保留；恢复Build84在父组件隐藏的返回桌面、退出、音频焦点、音频卸载入口。退出沿用既有确认和清理；Android专用项保留值并显示能力说明，不伪装成iOS可执行。未删除的字体、主题、语言、源管理、搜索、列表和备份等不做无关改写。
- 验证：增加独立HTTP/加密对端测试、主题前景检查、原生codec/桥接检查和设置保留审计；沿用官方同步服务、双向变更、断线重连、独立进程、播放/缓存与宽窄主题截图测试。构建结果以本次Actions及产物为准，不预写真机全场景通过。

## iOS / iPadOS 1.9.0 Build 84

### 修复与增加
- 修复歌单详情底部播放栏被放在侧栏容器之外的问题；横屏从右侧内容分隔线开始，窄窗口按实际内容宽度铺满。
- 当前播放歌曲和选择内容采用独立高对比底色、播放标记与字重，不再只靠主题主色；播放器播放列表打开、恢复数据或切歌后定位当前曲目，提供手动定位入口。主列表、导航及菜单同步增强选中状态。
- 设置增加外接键盘总开关及播放、进度、选择、导航分项；支持空格、方向键、Enter、⌘L、⌘1–5。输入框获得焦点时不覆盖其编辑按键；弹窗和前台页面分层路由，隐藏页面不响应。
- 修复数据同步地址输入后尚未失焦无法启用、同步方向窗口标识初始化竞争、取消未清理、UTF-8配对编码及异步压缩消息顺序；连接与订阅按当前会话隔离，支持断线重连和明确错误提示。增加本地网络权限说明与系统设置入口。保留上游v4协议，不自动选择覆盖用户数据的方向。
- 关于页和README区分skyhc的iOS/iPadOS移植、lyswhut官方上游及Q-1515前期移植；增加移植仓库、Releases、问题反馈入口。iOS版本按钮不再引导Android安装包；Issues未启用时明确提示并保留反馈文本，不错误跳转上游问题区。
- 隐藏iOS无实际作用的Android返回桌面、退出、音频焦点和音频卸载设置。可见数据同步、连接历史、权限和发布页操作增加失败反馈。

### 验证与边界
- 新增16项可执行回归，保留既有缓存、布局、音量、列表持久化与1.9.0兼容检查。
- Actions增加固定官方同步服务版本及独立桌面协议客户端；通过真实iOS HTTP/WebSocket、原生加密和持久化验证首次配对、错误码、双向增删改、压缩中文数据、断线重连及进程重启。不是对用户个人电脑和局域网设置的远程验收。
- 原生键盘命令设置、文本输入焦点和JS派发通过模拟器检查；外接实体键盘、第三方输入法和全部设备布局仍需安装测试。
- 持续保留MP3/FLAC起播、音量、暂停、跳转、缓存和离线重启检查。生产解码及音量业务逻辑不变。
- 具体构建结果以对应Actions运行与产物为准；本条不预先声明编译或真机测试通过。


## iOS / iPadOS 1.9.0 Build 83 · 2026-09-13

- 基于用户验收并已合并的 Build 82；固定官方 v1.9.0 提交 cd37a979a5845f1220b306b374285f5d38329e8d，保留上游合并历史。
- 移植歌曲菜单“清理缓存”、换源版本匹配、kg/tx 搜索、tx 歌单兼容和翻页、mg 图片/歌词、kw/wy/tx 歌词修正与纯 JS QRC 解码。
- 清理单曲缓存同时移除该曲全部音质的 URL 与 iOS 完整音频缓存；保持当前播放副本，不影响其他歌曲，不允许旧观察任务重新落盘。
- 保留原生 FLAC 与 TrackPlayer 分流、起播非阻塞、持久缓存、音量、锁屏歌词、Scene 窗口管理及已验收的 iPad UI。
- 保留来源标记、深色状态栏、协调表头、完整无暗幕菜单、居中收藏、侧栏统一尺寸、窄窗口手机版布局与底部导航隐藏设置。
- 接入上游完整蓝牙歌词设置，仅 Android 展示；iOS 沿用现有锁屏逐行歌词，不显示无效开关。
- 依赖与锁文件跟随官方 1.9.0，保留 iOS postinstall 补丁；iOS Build 递增为83，Bundle ID 不变，可覆盖升级而无需清空歌单或设置。
- 本地检查及 Actions 测试结果另见交付记录；未以编译成功替代实体设备、所有外部音源及蓝牙车机验证。
...
## iOS / iPadOS 1.8.2 Build 82 · 2026-09-13

### 修复

- 横屏“我的列表”和排行榜取消通用页头重复占位；常驻侧栏负责列表名称，歌曲工具栏不再重复当前列表名。
- 主题切换同步更新已存在的 React Native Navigation 控制器、后续页面默认值和原生窗口外观；深色主题使用浅色状态栏文字，弹窗关闭及窗口恢复时不遗留旧的浅色主题设置。
- 表头改为透明背景，沿用页面主题/背景图，取消深色背景上的实心灰条；时长表头与内容共用 70 点居中单元格，操作列保留独立间距。
- 操作菜单按完整本地化文案确定宽度，并用原生文本测量补足；窄窗口允许文字换行及菜单滚动，不再截断文案。“添加到…”/“移动到…”改为明确的列表操作名称。
- 接入单窗口 UIScene 生命周期，在创建 React Native 桥接前绑定原生 scene window；通过 Apple 公开接口固定使用 minimal 窗口控件样式，避免旧兼容样式的额外顶部区域。系统仍管理红绿灯的实际几何位置；不修改私有系统视图、不伪造红绿灯。
- 安全区按实际接收内容的根视图计算，移除全窗口坐标换算导致的重复区域；保留真实系统控件/状态栏遮挡范围，不用负边距硬裁内容。
- 二级菜单/弹窗改为透明交互遮罩，不再把整个窗口变暗；通过卡片背景、描边和四周阴影区分层级，仍可点击菜单外关闭。
- 恢复歌曲标题后的 kw/wy/kg 等音源标签，横屏及竖屏均显示；搜索、排行榜、我的列表和播放器列表共用显示组件。

### 验证与边界

新增 Build82 可执行布局/菜单/主题回归；保留缓存、音量与列表持久化测试。Actions 先运行 Release 模拟器 MP3/FLAC 播放和离线重启，再检查 scene/window 状态及浅深色切换，并生成 36 张生产组件截图，最后编译设备 Release Archive 和未签名 IPA。实际通过状态以对应提交的 Actions 日志为准，不能用此更新日志预先宣称成功。未调整生产音频解码、音量、缓存算法；原生窗口启动改变需通过播放/重启门禁。实体 iPad 任意浮动窗口位置、系统按钮外观和扬声器试听不能由截图或模拟器全部代替。
...
## iOS / iPadOS 1.8.2 Build 81 — 2026-09-13

### 修复与调整

- 顶部安全区改为从 UIWindow 的垂直布局区域转换到实际内容根坐标，只补足尚未消费的部分；取消应用额外的 36/40 点占位。iOS 页面标题不再另加状态栏高度。红绿灯继续由 iPadOS 定位，没有使用私有 API 移动系统控件，不能承诺跨系统状态固定绝对坐标。
- 合并竖屏／窄窗口“我的列表”的列表切换与搜索工具栏，移除重复的当前试听列表标题，保留搜索、列表操作及多选功能。
- iOS 歌曲列表在竖屏／窄窗口隐藏分列表头，使用歌曲名与“来源＋歌手”双行条目；横屏显示表头并将歌名、艺术家、专辑按相同弹性宽度分配。时长采用独立固定宽度，与操作按钮留出间隔，在线列表、排行榜、我的列表及播放列表共用规则。
- 操作菜单、下拉面板、弹窗与对话框统一使用遮罩、实色卡片、边框和 iOS 阴影；操作菜单以当前安全视口定位，避免窗口变化后出现错位或越界。
- 歌单详情和排行榜操作按钮采用不压缩的单行文字；极窄空间允许按钮组横向滚动，避免“播放全部”“收藏歌单”拆行。
- 设置 → 基本设置新增“竖屏／窄窗口隐藏底部导航”，默认关闭并持久保存。隐藏后仍可从顶部菜单访问所有页面，横屏侧栏不受影响。
- 本轮不修改音频解码、播放引擎、音量、缓存或列表删除逻辑；保留已有原生播放与离线重启回归门禁。

### 验证

新增 `check-build81-regressions.js` 覆盖安全区不叠加、竖横屏歌曲布局、菜单安全边界与层次、导航开关和单行操作栏。更新旧检查中已被新需求替代的布局预期，保留音频和持久化行为检查。Actions 运行全项目 TypeScript、Release 模拟器播放、18 张生产视图截图与设备端 Release archive；以对应运行结果和产物为准，不预先宣称构建或实体 iPad 全场景验收通过。

## iOS / iPadOS 1.8.2 Build 80 — 2026-09-13

### 修复与调整

- 增加 iOS 文本基准字号（常用正文 15→17 点，列表歌名 14→16 点），保持窗口缩放往返时字号稳定；图标尺寸不随本次文字调整增大。浅色/深色主题的正文、辅助文字和文字按钮使用经过对比度计算的语义前景色。
- 恢复“序号、歌曲名、艺术家、专辑名、时长、操作”表头，在线搜索/排行榜/歌单详情与我的列表共用列布局；表头位于滚动区域上方，空列表和加载中也保留。窄窗口按行实际宽度收起专辑/时长，不隐藏全部标题。
- 宽窗口排行榜增加常驻榜单标题、播放和收藏操作；歌单详情保留主导航栏，封面/介绍与操作按钮左右排列，窄窗口继续采用紧凑布局。
- 音量滑块仅在用户手动拖动时预览音量，忽略挂载/尺寸变化产生的数值回调，打开面板时重新应用已保存音量，取消拖动时还原。提供“恢复应用音量100%”显式操作，不自动调高用户系统音量。
- 已关闭的均衡器不再因残留非零频段设置而继续生效。FLAC 干湿通路使用明确独立总线，主混音器保持单位增益，应用音量只在一个混音级应用。增加模拟器后混音 PCM 幅度与音量线性测试；并不以播放进度代替输出电平验证。
- “本地音乐”从每次进入页面创建改为持久化的一次性初始化；移除未打开过的用户列表也清理元数据并退出已移除的活动列表。列表写入按序执行，过期读取及迟到的同步结果不能重建已移除的列表；移除失败显示错误，不报告成功。
- 收藏添加/多选添加改为居中、有界宽度的响应式按钮网格，宽窗口最多三列，提供虚线新建入口。只有明确标记为列表的面板从左侧滑入；列表面板统一上限420×600点，按可用窗口和键盘缩小。设置/音效/定时保持居中。

### 验证与范围

构建前执行缓存、窗口、列表持久化、对比度和音量手势回归；Actions 继续执行 Release 模拟器 MP3/FLAC、暂停恢复、切换和独立进程离线缓存播放，并新增输出增益、实际原生存储跨进程删除验证及 iPad/iPhone 生产组件截图。实际构建结果与产物以对应提交的 Actions 记录为准。模拟器电平不代表实体设备扬声器/蓝牙路由的主观响度。

### 后续修复记录规则

每次修复提交必须在此文件追加该版本的问题、实际处理和验证范围；代码检查不写成真机验收，未签名构建不写成可直接安装。

## iOS / iPadOS 1.8.2 Build 79 — 2026-09-13（补记）

- 分离缓存与起播等待，避免零秒跳转阻塞未就绪的音轨；保留本地缓存和两条播放引擎。
- 窄窗口恢复手机版布局，往返全屏实时更新尺寸；辅助图标统一对齐、缩小，播放列表使用独立图标。
- 我的列表改为常驻入口，iOS 歌曲列表改单行；增加 Release 模拟器在线与断网重启播放检查。

## iOS / iPadOS 1.8.2 Build 78 — 2026-09-13（补记）

- iPad 窗口控件采用垂直避让，播放器左右分栏。
- 实现完整音频持久缓存与本地 FLAC 入口；当时的测试未覆盖真实起播回归，相关阻塞已在 Build 79 处理。
